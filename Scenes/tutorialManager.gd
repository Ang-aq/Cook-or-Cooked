...
@onready var indicator: Node2D = $Right
...
@onready var ChecklistTarget = $ChecklistT
...
func _show_wrong_input_dialogue() -> void:
	...
	# Keep dialogue input locked during wrong input dialogue
	tutorial_dialog.input_locked = true
	tutorial_dialog.start_dialogue(wrong_prompt, portraits)
	await tutorial_dialog.dialogue_finished
	tutorial_dialog.input_locked = true  # keep locked until combo resumes
